...
class Chasha:
    def __init__(self):
        ...
        self.health = game_framework.player.atk ** 2 + 3 * game_framework.player.atk + 1
        self.xframe, self.yframe = 0, 4
        self.build_behavior_tree()

    def calculate_current_position(self):
        self.xframe = (self.xframe + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % FRAMES_PER_ACTION
        self.x += self.speed * math.cos(self.dir) * game_framework.frame_time
        self.y += self.speed * math.sin(self.dir) * game_framework.frame_time
        self.x = clamp(25, self.x, 480)
        self.y = clamp(25, self.y, 600)

    def draw(self):
        self.image.clip_draw(int(self.xframe) * self.charWidth, self.yframe * self.charHeight,
